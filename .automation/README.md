...
### Шаг 5. Запиши начальную точку синка
В корне репо создай файл `.sync-state.json` с последним известным ID:
```json
{"last_message_id": 73000}
```
Где `73000` — самое большое число `m\d+` в архиве. Найти можно так:
```bash
grep -hoE 'm[0-9]+' general-archive/*.md | sort -V | tail -1
```
Закоммить + push.
...
### Шаг 7. Проверь sync
**Actions → Sync Telegram → Run workflow → Run**. Через минуту проверь — если есть новые сообщения, в репо появится коммит «Sync: ...».

Дальше синк работает сам, каждые 6 часов. Деплой сайта на каждый коммит — тоже автоматически.
